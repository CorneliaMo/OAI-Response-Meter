...
func TestDecodeValidUsage(t *testing.T) {
	...
	if got.ResponseID != "resp_123" || got.PreviousResponseID != "resp_parent" || got.ChainRootResponseID != "resp_root" || got.PromptCacheKey != "session-uuid" || got.TotalTokens != 30 || got.CacheWriteTokens != 2 {
		t.Fatalf("Decode() = %+v", got)
	}
}

func TestDecodeDefaultsMissingCacheWriteTokensToZero(t *testing.T) {
	raw := []byte(`{
		"schema": 1,
		"ts": "2026-06-20T12:00:00Z",
		"source": "mitmproxy",
	...
	if got.CacheWriteTokens != 0 {
		t.Fatalf("CacheWriteTokens = %d, want 0", got.CacheWriteTokens)
	}
}
...
func TestDecodeRejectsInvalidUsage(t *testing.T) {
	tests := map[string]string{
		...
		"negative tokens":     `{"schema":1,"ts":"2026-06-20T12:00:00Z","transport":"sse","host":"api.openai.com","response_id":"resp_123","cache_write_tokens":-1}`,
	}
